...
// ═══════════════════════════════════════════════════════════════════════════
// MIDTRANS CORE API — Direct / Embedded Checkout (tanpa Snap popup)
// ═══════════════════════════════════════════════════════════════════════════

/**
 * Mapping payment_method yang dikirim frontend → konfigurasi body Core API
 * Setiap metode memiliki payment_type & sub-field spesifik sesuai dokumentasi Midtrans.
 */
export const PAYMENT_METHOD_CONFIG = {
  // ── QRIS ──────────────────────────────────────────────────────────────
  qris:        { payment_type: 'qris',      extra: {} },
  gopay_qris:  { payment_type: 'qris',      extra: { qris: { acquirer: 'gopay' } } },

  // ── E-Wallet ──────────────────────────────────────────────────────────
  gopay:       { payment_type: 'gopay',     extra: {} },
  shopeepay:   { payment_type: 'shopeepay', extra: {} },
  dana:        { payment_type: 'gopay',     extra: {} }, // fallback via gopay channel

  // ── Virtual Account (Bank Transfer) ───────────────────────────────────
  bca_va:      { payment_type: 'bank_transfer', extra: { bank_transfer: { bank: 'bca'  } } },
  ...
  other_va:    { payment_type: 'bank_transfer', extra: { bank_transfer: { bank: 'bni'  } } }, // default BNI
};
...
export async function createCoreTransaction({ orderId, amount, playerUsername, productName, paymentMethod }) {
  ...
  const body = {
    ...
    customer_details:    { first_name: playerUsername, email: `${playerUsername}@fancynet.my.id` },
    item_details: [{ id: orderId, price: amount, quantity: 1, name: productName.substring(0, 50) }],
    // callbacks untuk deep-link redirect (GoPay/ShopeePay)
    gopay:     { enable_callback: true, callback_url: `${siteUrl}/invoice/${orderId}` },
    shopeepay: { callback_url: `${siteUrl}/invoice/${orderId}` },
    ...cfg.extra,
  };
 ...
 * @returns {{ vaNumber?, qrUrl?, deeplinkUrl?, billKey?, billCode? }}
 */
export function extractPaymentInfo(coreData, paymentMethod) {
  const info = {};
  if (paymentMethod === 'mandiri_va') {
    info.billKey  = coreData.bill_key;
    info.billCode = coreData.biller_code;
  } else if (paymentMethod === 'qris' || paymentMethod === 'gopay_qris') {
    info.qrUrl = coreData.actions?.find(a => a.name === 'generate-qr-code')?.url
              || coreData.qr_string;
  } else if (paymentMethod === 'gopay') {
    info.deeplinkUrl = coreData.actions?.find(a => a.name === 'deeplink-redirect')?.url;
    info.qrUrl       = coreData.actions?.find(a => a.name === 'generate-qr-code')?.url;
