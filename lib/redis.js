 ...
 *
 * Mendukung 3 format env var Upstash:
 *  1. Storage_KV_REST_API_URL + Storage_KV_REST_API_TOKEN  ← Vercel KV store bernama "Storage"
 *  2. KV_REST_API_URL + KV_REST_API_TOKEN                  ← Vercel KV store default (tanpa nama)
 *  3. UPSTASH_REDIS_REST_URL + UPSTASH_REDIS_REST_TOKEN    ← Manual / legacy
 */
...
export function getRedis() {
  ...
  // Coba semua kemungkinan nama env var — urutan prioritas
  const url =
    process.env.Storage_KV_REST_API_URL   ||   // Vercel KV "Storage"
    process.env.KV_REST_API_URL            ||   // Vercel KV default
    process.env.UPSTASH_REDIS_REST_URL;         // Manual

  const token =
    process.env.Storage_KV_REST_API_TOKEN  ||   // Vercel KV "Storage"
    process.env.KV_REST_API_TOKEN          ||   // Vercel KV default
    process.env.UPSTASH_REDIS_REST_TOKEN;       // Manual
  ...
  try {
    ...
    console.log('[redis] Connected:', url.split('.')[0].replace('https://', ''));
  } catch (e) {
...
async function kvGet(redisKey, fallbackFile, defaultVal) {
  ...
  if (r) {
    try {
      ...
      // Redis kosong — seed dari file (migrasi pertama kali)
...
export const OrdersAsync = {
  ...
  async delete(order_id) {
    const orders = (await OrdersAsync.all()).filter(o => o.order_id !== order_id);
    await OrdersAsync.save(orders);
  },
...
// ── Tickets (individual keys untuk performa) ─────────────────────
