...
const DEFAULTS = {
  ...
  'leaderboard.json': { balance: [], auraskills: [], votes: [], lastSync: {} },
};
...
export const Products = {
  ...
  add(prod) {
    ...
    // Respect is_active from input; default true if not specified
    const isActive = prod.is_active !== undefined ? (prod.is_active === true || prod.is_active === 1) : true;
    const newP  = { ...prod, id: Products.nextId(), slug, is_active: isActive, created_at: new Date().toISOString() };
    prods.push(newP);
    Products.save(prods);
    return newP;
  },
...
export const Tickets = {
  ...
  add(ticket) {
    ...
    all.push({ ...ticket, status:'open', messages:[], created_at: new Date().toISOString(), updated_at: new Date().toISOString() });
    Tickets.save(all);
  },
...
export const Leaderboard = {
  get:   ()             => readData('leaderboard.json'),
  board: (name)         => Leaderboard.get()[name] || [],

  setBoard(name, entries) {
