...
function resolveDataDir() {
  ...
  if (fs.existsSync('/app')) return '/app/data';
...
export const Categories = {
  ...
  active: ()     => Categories.all().filter(c => c.is_active),
...
export const Products = {
  ...
  active: ()   => Products.all().filter(p => p.is_active),
...
export const Tickets = {
  ...
  add(ticket) {
    ...
    all.push({ ...ticket, status:'open', created_at: new Date().toISOString(), updated_at: new Date().toISOString() });
    Tickets.save(all);
  },
