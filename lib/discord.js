...
function buildPremiumEmbed(order, cfg) {
  ...
  return {
    author: {
      name: `${cfg.labelId}  ·  NexusCloud.id`,
      icon_url: skinUrl,
    },
    title: null,
    color: cfg.color,
    thumbnail: { url: skinUrl },
    description: `## ${cfg.dot}  ${cfg.labelId}\nTerima kasih sudah melakukan transaksi di **NexusCloud**.\nPesanan kamu telah berhasil diproses dan tercatat sebagai transaksi resmi NexusCloud.`,
    fields,
    timestamp: new Date().toISOString(),
    footer: {
      text: 'NexusCloud.id  •  Trusted Transaction',
...
function buildAdminEmbed(order, cfg) {
  ...
  return {
    ...
    timestamp: new Date().toISOString(),
    footer: {
      text: 'NexusCloud.id  •  Admin Log System',
      icon_url: 'https://minotar.net/helm/MHF_Steve/16',
    },
  };
}
...
async function generateInvoicePdf(order) {
  ...
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      size: 'A4', margin: 50,
      info: { Title: `Invoice #${order.order_id}`, Author: 'NexusCloud Store' },
    });
    const chunks = [];
    ...
    doc.fontSize(10).fillColor(GRAY).font('Helvetica').text('NexusCloud.id • Trusted Minecraft Store', LEFT, 78);
    ...
    doc.fontSize(8).fillColor(GRAY).font('Helvetica')
       .text('Dokumen ini digenerate otomatis oleh NexusCloud Store · Simpan sebagai bukti pembayaran resmi.', LEFT, y, { width: W, align: 'center' });
...
export async function webhookTransaction(order) {
  ...
  if (isSuccess && playerUrl) {
    ...
    const customerName  = order.discord_username || order.player_username || '-';
    const playerContent = `🎉 Hei **${customerName}**! Transaksi kamu berhasil diproses oleh **NexusCloud**. ✨`;
...
export async function webhookReport(ticket) {
  ...
  await sendEmbed(url, {
    ...
    embeds: [{
      ...
      timestamp: new Date().toISOString(),
      footer: { text: 'NexusCloud Support' },
    }],
  });
}
...
export async function webhookTicketArchive(ticket) {
  ...
  await sendFile(url, `${ticket.ticket_id}.txt`, Buffer.from(lines, 'utf-8'), 'text/plain; charset=utf-8', {
    ...
    embeds: [{
      ...
      timestamp: new Date().toISOString(),
      footer: { text: 'NexusCloud Support — Auto Cleanup' },
    }],
  });
}
...
export async function webhookOrderArchive(orders) {
  ...
  const lines = [
    'NexusCloud Store — Arsip Log Transaksi',
  ...
  await sendFile(url, filename, Buffer.from(lines.join('\n'), 'utf-8'), 'text/plain', {
    ...
    embeds: [{
      ...
      timestamp: new Date().toISOString(),
      footer: { text: 'NexusCloud Store — Auto Cleanup System' },
    }],
  });
}
