...
async function generateInvoicePdf(order) {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(10).fillColor(GRAY).font('Helvetica').text('Premium Minecraft Server Shop', LEFT, 78);
    ...
    for (const [lbl, val] of details) {
      doc.fontSize(9).fillColor(GRAY).font('Helvetica').text(lbl+':', col2X, ry, { continued:true, width:80 })
         .fillColor(BLACK).text(' '+val, { width: RIGHT-col2X-10 });
      ry += 13;
    }
    ...
    doc.fontSize(9).fillColor(GRAY).font('Helvetica').text('Permanen · Lifetime', col.desc, y+22, { width:W*0.5 });
