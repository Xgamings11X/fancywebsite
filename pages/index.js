  ...
  return (
    <>
      <Head>
        ...
        {/* Favicon: dinamis dari LogoImage (transparent PNG) */}
        ...
        <div style={{margin:'0 auto 32px',display:'flex',alignItems:'center',justifyContent:'center'}}>
          {s.logo_url
            ? <img src={s.logo_url} alt={serverName} style={{
                ...
                filter:'drop-shadow(0 8px 32px rgba(255,107,0,0.45))',
                animation:'logoFloat 3s ease-in-out infinite',
              }}/>
            : <LogoImage alt={serverName} style={{
                width:160,height:160,objectFit:'contain',display:'block',
                filter:'drop-shadow(0 8px 32px rgba(255,107,0,0.55)) drop-shadow(0 0 60px rgba(255,107,0,0.2))',
                animation:'logoFloat 3s ease-in-out infinite',
              }}/>
          }
        </div>
        ...
        <div style={{maxWidth:580,margin:'0 auto'}}>
          <div style={{background:'linear-gradient(145deg,rgba(20,20,27,0.8),rgba(10,10,14,0.8))',border:'1px solid var(--border)',borderRadius:20,padding:40,position:'relative',overflow:'hidden'}}>
            ...
            <a href={famousApplyUrl} target="_blank" rel="noopener noreferrer"
              style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',color:'#fff',padding:14,borderRadius:10,fontWeight:600,fontSize:13,cursor:'pointer',width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:8,textDecoration:'none',transition:'all 0.2s'}}
